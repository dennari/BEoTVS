...
% Enjoy life!!!
    ...
    okargs = {'rowlabels','columnlabels', 'alignment', 'format', 'size',...
        'rowlabelalignment','columnlabelalignment'};
    for j=1:2:(nargin-2)
        pname = varargin{j};
        pval = varargin{j+1};
        ...
        elseif length(k)>1
            ...
            switch(k)
                ...
                case 3  % alignment
                    alignment = pval;
                    if(ischar(alignment))
                        alignment = cellstr(repmat(alignment,width,1));
                    end
                case 4  % format
                    format = lower(pval);
                    if(ischar(format))
                        format = cellstr(repmat(format,width,1));
                    end
                case 5  % format
                    textsize = pval;
                case 6  % rowlabelalignment
                    rlAlignment = pval;
                case 7  % columnlabelalignment
                    clAlignment = pval;    
            end
        end
    end
    ...
    maxlengths = ones(1,width);
    if isnumeric(matrix)
        matrix = num2cell(matrix);
        for h=1:height
            for w=1:width
                v = matrix{h, w};
                % maximum number of integers per column
                if(~isnan(v)) 
                    intl = length(num2str(round(abs(v))));
                    if intl > maxlengths(w)
                        maxlengths(w) = intl;
                    end
                end
                if(isnan(v)) 
                    matrix{h, w} = '\multicolumn{1}{c}{}';
                elseif(~isempty(format))
                    matrix{h, w} = num2str(v, format{w});
                else
                    matrix{h, w} = num2str(v);
                end
            end
        end
    end
    ...
    if(~isempty(rowLabels))
        fprintf(fid, rlAlignment);
    end
    for i=1:width
        % if we want automatic counting, do it
        al = regexprep(alignment{i},'d{\?}',sprintf('d{%.f}',maxlengths(i)));
        fprintf(fid, '%s', al);
    end
    ...
    if(~isempty(colLabels))
        %fprintf(fid, '\\rowstyle{\\bfseries}\r\n');
        if(~isempty(rowLabels))
            fprintf(fid, '&');
        end
        for w=1:width-1
            fprintf(fid, '\\multicolumn{1}{%s}{{\\bfseries %s}}&',clAlignment, colLabels{w});
        end
        fprintf(fid, '\\multicolumn{1}{%s}{{\\bfseries %s}}\\\\\\otoprule\r\n',clAlignment, colLabels{width});
    end
    
    for h=1:height
        if(~isempty(rowLabels))
            fprintf(fid, '{\\bfseries %s}&', rowLabels{h});
        end
        for w=1:width-1
